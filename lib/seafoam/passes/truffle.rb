    ...
    class TrufflePass < Pass
      ...
      def simplify_truffle_args(graph)
        graph.nodes.dup.each_value do |node|
          ...
          arg_node =
            graph.create_node(graph.new_id, { synthetic: true, inlined: true, label: "T(#{index})", kind: "input" })
        ...
        commit_allocation_nodes.each do |commit_allocation_node|
          ...
          objects.each do |new_node, virtual_node, fields, values|
            ...
            if allocated_object_node
              ...
              allocated_object_node.outputs.each { |edge| graph.create_edge(new_node, edge.to, edge.props) }
            ...
            fields.zip(values) do |field, value_id|
              ...
              if @options[:hide_null_fields] and value_node.node_class == "org.graalvm.compiler.nodes.ConstantNode" and
